...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _initialiseProps = function _initialiseProps() {
	    ...
	    this._updateProps = function (props) {
	        _this2.isIE = !(_this2.checkIE() < 0 || _this2.checkIE() >= 10);
	        var options = props.options;
	        _this2.baseUrl = options.baseUrl; //域名
	        _this2.param = options.param; //get参数
	        _this2.chooseAndUpload = options.chooseAndUpload || false; //是否在用户选择了文件之后立刻上传
	        _this2.paramAddToField = options.paramAddToField || undefined; //需要添加到FormData的对象。不支持IE
	        /*upload success 返回resp的格式*/
	        _this2.dataType = 'json';
	        options.dataType && options.dataType.toLowerCase() == 'text' && (_this2.dataType = 'text');
	        _this2.wrapperDisplay = options.wrapperDisplay || 'inline-block'; //包裹chooseBtn或uploadBtn的div的display
	        _this2.timeout = typeof options.timeout == 'number' && options.timeout > 0 ? options.timeout : 0; //超时时间
	        _this2.accept = options.accept || ''; //限制文件后缀
	        _this2.multiple = options.multiple || false;
	        _this2.numberLimit = options.numberLimit || false; //允许多文件上传时，选择文件数量的限制
	        _this2.fileFieldName = options.fileFieldName || false; //文件附加到formData上时的key，传入string指定一个file的属性名，值为其属性的值。不支持IE
	        _this2.withCredentials = options.withCredentials || false; //跨域时是否使用认证信息
	        _this2.requestHeaders = options.requestHeaders || false; //要设置的请求头键值对
	         ...
	         */
	        _this2.beforeChoose = options.beforeChoose || emptyFunction;
	        /**
	         * chooseFile(file) : 用户选择文件后的触发的回调函数
	         * @param file {File | string} 现代浏览器返回File对象，IE返回文件名
	         * @return
	         */
	        _this2.chooseFile = options.chooseFile || emptyFunction;
	        /**
	         ...
	         * }), 则对本次的param进行处理
	         */
	        _this2.beforeUpload = options.beforeUpload || emptyFunction;
	        /**
	         ...
	         * @return
	         */
	        _this2.doUpload = options.doUpload || emptyFunction;
	        /**
	         ...
	         * @return
	         */
	        _this2.uploading = options.uploading || emptyFunction;
	        /**
	         * uploadSuccess(resp) : 上传成功后执行的回调（针对AJAX而言）
	         * @param resp {json | string} 根据options.dataType指定返回数据的格式
	         * @return
	         */
	        _this2.uploadSuccess = options.uploadSuccess || emptyFunction;
	        /**
	         * uploadError(err) : 上传错误后执行的回调（针对AJAX而言）
	         * @param err {Error | object} 如果返回catch到的error，其具有type和message属性
	         * @return
	         */
	        _this2.uploadError = options.uploadError || emptyFunction;
	        /**
	         * uploadFail(resp) : 上传失败后执行的回调（针对AJAX而言）
	         * @param resp {string} 失败信息
	         */
	        _this2.uploadFail = options.uploadFail || emptyFunction;
	        /**
	         * onabort(mill, xhrID) : 主动取消xhr进程的响应
	         * @param mill {long} 毫秒数，本次上传时刻的时间
	         * @param xhrID {int} 在doUpload时会返回的当次xhr代表ID
	         */
	        _this2.onabort = options.onabort || emptyFunction;

	        _this2.files = options.files || _this2.files || false; //保存需要上传的文件
	        /*特殊内容*/

	        /*IE情况下，由于上传按钮被隐藏的input覆盖，不能进行disabled按钮处理。
	         * 所以当disabledIEChoose为true（或者func返回值为true）时，禁止IE上传。
	         */
	        _this2.disabledIEChoose = options.disabledIEChoose || false;

	        _this2._withoutFileUpload = options._withoutFileUpload || false; //不带文件上传，为了给秒传功能使用，不影响IE
	        _this2.filesToUpload = options.filesToUpload || []; //使用filesToUpload()方法代替
	        _this2.textBeforeFiles = options.textBeforeFiles || false; //make this true to add text fields before file data
	        /*使用filesToUpload()方法代替*/
	        if (_this2.filesToUpload.length && !_this2.isIE) {
	            _this2.filesToUpload.forEach(function (file) {
	                _this2.files = [file];
	                _this2.commonUpload();
	            });
	        }
	    ...
	    this.commonChange = function (e) {
	        ...
	        /*如果限制了多文件上传时的数量*/
	        var numberLimit = typeof _this2.numberLimit === 'function' ? _this2.numberLimit() : _this2.numberLimit;
	        if (_this2.multiple && numberLimit && files.length > numberLimit) {
	            var newFiles = {};
	        ...
	        _this2.files = files;
	        _this2.chooseFile(files);
	        _this2.chooseAndUpload && _this2.commonUpload();
	    };

	    this.commonUpload = function () {
	        /*mill参数是当前时刻毫秒数，file第一次进行上传时会添加为file的属性，也可在beforeUpload为其添加，之后同一文件的mill不会更改，作为文件的识别id*/
	        var mill = _this2.files.length && _this2.files[0].mill || new Date().getTime();
	        var jud = _this2.beforeUpload(_this2.files, mill);
	        if (jud != true && jud != undefined && (typeof jud === 'undefined' ? 'undefined' : _typeof(jud)) != 'object') {
	            /*清除input的值*/
	            _this2.refs['ajax_upload_file_input'].value = '';
	            return;
	        }

	        if (!_this2.files) return;
	        if (!_this2.baseUrl) throw new Error('baseUrl missing in options');
	        ...
	        if (_this2.textBeforeFiles) {
	            formData = _this2.appendFieldsToFormData(formData);
	        }
	        if (!_this2._withoutFileUpload) {
	        ...
	        var baseUrl = _this2.baseUrl;

	        /*url参数*/
	        /*如果param是一个函数*/
	        var param = typeof _this2.param === 'function' ? _this2.param(_this2.files) : _this2.param;
	        ...
	        /*跨域是否开启验证信息*/
	        xhr.withCredentials = _this2.withCredentials;
	        /*是否需要设置请求头*/
	        var rh = _this2.requestHeaders;
	        ...
	        /*不带文件上传，给秒传使用*/
	        _this2._withoutFileUpload ? xhr.send(null) : xhr.send(formData);
	        ...
	        /*trigger执行上传的用户回调*/
	        _this2.doUpload(_this2.files, mill, currentXHRID);

	        /*清除input的值*/
	        _this2.refs['ajax_upload_file_input'].value = '';
	    };

	    this.appendFieldsToFormData = function (formData) {
	        var field = typeof _this2.paramAddToField == 'function' ? _this2.paramAddToField() : _this2.paramAddToField;
	    ...
	    this.IEBeforeChoose = function (e) {
	        var jud = _this2.beforeChoose();
	        jud != true && jud != undefined && e.preventDefault();
	    };

	    this.IEChooseFile = function (e) {
	        _this2.fileName = e.target.value.substring(e.target.value.lastIndexOf('\\') + 1);
	        _this2.chooseFile(_this2.fileName);
	        /*先执行IEUpload，配置好action等参数，然后submit*/
	        _this2.chooseAndUpload && _this2.IEUpload() !== false && document.getElementById('ajax_upload_file_form_' + _this2.IETag + currentIEID).submit();
	        e.target.blur();
	    };

	    this.IEUpload = function (e) {
	        var mill = new Date().getTime();
	        var jud = _this2.beforeUpload(_this2.fileName, mill);
	        if (!_this2.fileName || jud != true && jud != undefined) {
	            e && e.preventDefault();
	            return false;
	        }
	        var that = _this2;
	        ...
	        var param = typeof _this2.param === 'function' ? _this2.param(_this2.fileName) : _this2.param;
	        var paramStr = '';
	        ...
	        document.getElementById('ajax_upload_file_form_' + _this2.IETag + currentIEID).setAttribute('action', targeturl);
	        /*IE假的上传进度*/
	        var getFakeProgress = _this2.fakeProgress();
	        var loaded = 0,
	            count = 0;
	        ...
	        /*回调函数*/
	        window.attachEvent ? document.getElementById('ajax_upload_file_frame_' + _this2.IETag + partIEID).attachEvent('onload', handleOnLoad) : document.getElementById('ajax_upload_file_frame_' + _this2.IETag + partIEID).addEventListener('load', handleOnLoad);
	        ...
	        _this2.doUpload(_this2.fileName, mill);
	        /*置为非空闲*/
	        IEFormGroup[currentIEID] = false;
	    };

	    this.IECallback = function (dataType, frameId) {
	        /*回复空闲状态*/
	        IEFormGroup[frameId] = true;

	        var frame = document.getElementById('ajax_upload_file_frame_' + _this2.IETag + frameId);
	        var resp = {};
	    ...
	    this.forwardChoose = function () {
	        if (_this2.isIE) return false;
	        _this2.commonChooseFile();
	    };

	    this.fowardRemoveFile = function (func) {
	        _this2.files = func(_this2.files);
	    };

	    this.filesToUpload = function (files) {
	        if (_this2.isIE) return;
	        _this2.files = files;
	        _this2.commonUpload();
	    };
	    ...
	    this.checkIE = function () {
	        var userAgent = _this2.userAgent;
	    ...
	    this.getUserAgent = function () {
	        var userAgentString = _this2.props.options && _this2.props.options.userAgent;
	    ...
	    this.componentWillMount = function () {
	        _this2.userAgent = _this2.getUserAgent();
	        _this2.isIE = !(_this2.checkIE() < 0 || _this2.checkIE() >= 10);
	        /*因为IE每次要用到很多form组，如果在同一页面需要用到多个<FileUpload>可以在options传入tag作为区分。并且不随后续props改变而改变*/
	        var tag = _this2.props.options && _this2.props.options.tag;
	        _this2.IETag = tag ? tag + '_' : '';

	        _this2._updateProps(_this2.props);
	    };

	    this.componentWillReceiveProps = function (newProps) {
	        _this2._updateProps(newProps);
	    };

	    this.render = function () {
	        return _this2._packRender();
	    };

	    this._packRender = function () {
	        /*IE用iframe表单上传，其他用ajax Formdata*/
	        var render = '';
	        if (_this2.isIE) {
	            render = _this2._multiIEForm();
	        } else {
	            var restAttrs = {
	            ...
	            render = React.createElement(
	                'div',
	                { className: _this2.props.className, style: _this2.props.style },
	                _this2.state.before,
	                React.createElement(
	                    'div',
	                    { onClick: _this2.commonChooseFile,
	                        style: { overflow: 'hidden', postion: 'relative', display: _this2.wrapperDisplay }
	                    },
	                    _this2.state.chooseBtn
	                ),
	                _this2.state.middle,
	                React.createElement(
	                    'div',
	                    { onClick: _this2.commonUpload,
	                        style: {
	                            overflow: 'hidden',
	                            postion: 'relative',
	                            display: _this2.chooseAndUpload ? 'none' : _this2.wrapperDisplay
	                        }
	                    },
	                    _this2.state.uploadBtn
	                ),
	                _this2.state.after,
	                React.createElement('input', _extends({ type: 'file', name: 'ajax_upload_file_input', ref: 'ajax_upload_file_input',
	                    style: { display: 'none' }, onChange: _this2.commonChange
	                }, restAttrs))
	            );
	        }
	        return render;
	    ...
	    this._multiIEForm = function () {
	         ...
	         * 所以当disabledIEChoose为true（或者func返回值为true）时，禁止IE上传。
	         */
	        var isDisabled = typeof _this2.disabledIEChoose === 'function' ? _this2.disabledIEChoose() : _this2.disabledIEChoose;

	        /*这里IEFormGroup的长度会变，所以不能存len*/
	        for (var i = 0; i < IEFormGroup.length; i++) {
	            _insertIEForm.call(_this2, formArr, i);
	        ...
	        return React.createElement(
	            'div',
	            { className: _this2.props.className, style: _this2.props.style, id: 'react-file-uploader' },
	            formArr
	        );
